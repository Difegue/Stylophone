...
namespace Stylophone.Services
{
    ...
    {
        public Imaging.BitmapImage ArtBitmap { get; set; }
    ...
    public static class AlbumArtService
    {
        ...
        public async static Task<AlbumArt> GetAlbumArtAsync(IMpdFile f, bool calculateDominantColor, int albumArtWidth, CoreDispatcher dispatcher = null)
        {
            ...
            if (bitmap != null)
            {
                ...
                {
                    result.DominantColor = new QuantizedColor(new Color(), 1);
        ...
        private async static Task<Imaging.WriteableBitmap> GetAlbumBitmap(IMpdFile f, CoreDispatcher dispatcher, CancellationToken token = default)
        {
            Imaging.WriteableBitmap result;
        ...
        private static async Task<Imaging.BitmapImage> WriteableBitmapToBitmapImageAsync(Imaging.WriteableBitmap art, int decodedPixelWidth, CoreDispatcher dispatcher)
        {
            Imaging.BitmapImage image = null;
            await dispatcher.AwaitableRunAsync(async () =>
            {
                image = new Imaging.BitmapImage();
        ...
        private static async Task<Imaging.WriteableBitmap> LoadImageFromFile(string fileName, CoreDispatcher dispatcher)
        {
            try
            {
                StorageFolder pictureFolder = await ApplicationData.Current.LocalFolder.CreateFolderAsync("AlbumArt", CreationCollisionOption.OpenIfExists);
        ...
        private async static Task<Imaging.WriteableBitmap> ImageFromBytes(byte[] bytes, CoreDispatcher dispatcher)
        {
            ...
            using (var stream = new MemoryStream(bytes))
            {
                await dispatcher.AwaitableRunAsync(async () =>
                {
                    image = Imaging.BitmapFactory.FromStream(stream);

                    // Resize overly large images to reduce OOM risk. Is 2048 too small ?
                    if (image.PixelWidth > 2048)
                    {
                        image = Imaging.WriteableBitmapExtensions.Resize(image, 2048, 2048 * image.PixelHeight / image.PixelWidth, Imaging.WriteableBitmapExtensions.Interpolation.Bilinear);
                    }
                });
            }
#endif
